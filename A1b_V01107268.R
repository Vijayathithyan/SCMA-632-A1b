...
setwd('D:\\MDA\\Course\\Boot Camp\\SCMA 632\\Assignments\\A1b')
...
# Fit distribution for Arshdeep Singh
Arshdeep_singh_wickets <- last_three_seasons %>%
  filter(Bowler == "Arshdeep Singh") %>%
  dplyr::select(wickets)

# Check if the resulting wickets are numeric and have more than one element
if (is.numeric(Arshdeep_singh_wickets$wickets) && length(Arshdeep_singh_wickets$wickets) > 1) {
  Arshdeep_singh_wickets <- fitdist(Arshdeep_singh_wickets$wickets, "norm")
  print(summary(Arshdeep_singh_wickets))
} else {
  print("Arshdeep Singh's wickets are not a numeric vector of length greater than 1.")
}
